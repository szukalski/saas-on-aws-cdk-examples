...
import { TicklesService } from "../TicklesService";
import {
  ServerSerdeContext,
...
export type HelloErrors = ValidationException | InternalServerError

export class HelloSerializer implements __OperationSerializer<TicklesService<any>, "Hello", HelloErrors> {
  serialize = serializeHelloResponse;
  deserialize = deserializeHelloRequest;

  isOperationError(error: any): error is HelloErrors {
    const names: HelloErrors['name'][] = ["ValidationException", "InternalServerError"];
...
export const getHelloHandler = <Context>(operation: __Operation<HelloServerInput, HelloServerOutput, Context>): __ServiceHandler<Context, __HttpRequest, __HttpResponse> => {
  const mux = new httpbinding.HttpBindingMux<"Tickles", "Hello">([
...
export class HelloHandler<Context> implements __ServiceHandler<Context> {
  ...
  private readonly mux: __Mux<"Tickles", "Hello">;
  private readonly serializer: __OperationSerializer<TicklesService<Context>, "Hello", HelloErrors>;
  ...
  constructor(
    ...
    mux: __Mux<"Tickles", "Hello">,
    serializer: __OperationSerializer<TicklesService<Context>, "Hello", HelloErrors>,
    serializeFrameworkException: (e: __SmithyFrameworkException, ctx: __ServerSerdeContext) => Promise<__HttpResponse>,
  ...
  async handle(request: __HttpRequest, context: Context): Promise<__HttpResponse> {
    ...
    if (target === undefined) {
      console.log('Received a request that did not match com.saasonaws#Tickles.Hello. This indicates a misconfiguration.');
